...
let q1 = ["한식", "양식", "3만원 이하", "안 매움", "0~500kcal", "다인분", "국물 없음", "술 안주"];  //왼쪽 질문 모음
let q2 = ["일식", "아시안", "3만원 이상", "매움", "500~1000kcal", "1인분", "국물 있음", "식사"];    //오른쪽 질문 모음
...
class Section2 extends Component {
    ...
    constructor(probs) {    //state 클래스버전으로 사용 
        super(probs);
        this.state = {
            count: 0
        }
        ...
        return (
            ...
            <div>
                ...
                <div className="question">
                    ...
                    <div className="question_1" onClick={() => {// 왼쪽 질문 클릭 시 
                        console.log(this.state.count);  //테스트용 

                        if (this.state.count < q1.length + 1) { //count가 q1배열의 길이보다 작을동안, 즉 질문이 다 끝날때동안

                            results.push(q1[this.state.count]); //클릭된 질문을 results 배열의 뒤에 삽입

                            if (this.state.count <= 1) {    //첫번째, 두번째 한식,양식,일식,아시안 중에 묻는 질문은
                                q3.push(results[this.state.count])  //q3에 따로 저장
                            }
                            if (this.state.count > 1 && check1 == true) {   //q3에 있는 질문을 다시 q1배열의 맨 뒤에 삽입 (4개의 국가 음식 중 하나를 뽑아내기 위함)
                                q1.push(q3[0]);
                            ...
                            if (this.state.count > 7) { //모든 질문이 끝나면 vs를 요리중으로 변경 
                                vs = "요리중";
                                results.shift();
                                results.shift();
                                let tmp = results.pop();
                    ...
                    }}> <h2>{q1[this.state.count]} </h2></div>
                    <div className="vs"> {vs} </div>
                    <div className="question_2" onClick={() => {    //위와 동일한 알고리즘
